...
for region in regions:
    for split in splits:
        for normal in normals:
            ...
            #Fit ICA
            ica_test = FastICA(n_components=100, algorithm='parallel', whiten='arbitrary-variance')
            ...
            ica_95 = FastICA(n_components=n_components_95, algorithm='parallel', whiten='arbitrary-variance')
            ica_90 = FastICA(n_components=n_components_90, algorithm='parallel', whiten='arbitrary-variance')
    ...
    for split in splits:
        for normal in normals:
            for window in windows:
                ...
                out_train_name_90 = f"{region}_{split}_{normal}_{window}_ICA_90_train.csv"
                out_test_name_90 = f"{region}_{split}_{normal}_{window}_ICA_90_test.csv"
                out_train_name_95 = f"{region}_{split}_{normal}_{window}_ICA_95_train.csv"
                out_test_name_95 = f"{region}_{split}_{normal}_{window}_ICA_95_test.csv"

                train_df = pd.read_csv("../data/encoded/" + region + "_" + split + "_" + normal + "_" + window + "_train.csv")
                test_df = pd.read_csv("../data/encoded/" + region + "_" + split + "_" + normal + "_" + window + "_test.csv")
                # Preserve TOD columns
                train_TOD = train_df['TOD']
                test_TOD = test_df['TOD']
                # Drop TOD from dataframes pre reduction
                ...
                if n_components_95 == 100:
                    print("WARNING: n_components_95 = ", n_components_95)
                n_components_90 = np.argmax(np.cumsum(explained_variance_ratio) >= 0.90) + 1
...
# For Option 3.5:____________________________________________________________
folders = ["flatten_w1_conv/", "flatten_w2_conv/", "flatten_w3_conv/"]
regions = ["BA11", "BA47"]
splits = ["80", "70", "60"]
normals = ["log", "MM"]
    ...
    for region in regions:
        for split in splits:
            for normal in normals:
                ...
                # write to csv files - MAKE SURE TO CHANGE DESTINATION IF NECESSARY
                S_ica_95_train.to_csv("../data/reduced_CNN_flatten/"+out_train_name_90, index=False)
                S_ica_95_test.to_csv("../data/reduced_CNN_flatten/"+out_test_name_90, index=False)
