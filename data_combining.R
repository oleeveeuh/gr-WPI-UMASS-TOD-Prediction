...
# read in data
patient_info_raw <- read.csv("data/raw_data/GSE71620_Phenotype_GEO.csv")
samples_raw <- read.csv("data/raw_data/all_sample.csv")
# note this file has been edited to get rid of the info at the top before the actual data
gene_info <- read.csv("data/raw_data/gene_names.csv")
...
# Get rows with recorded TOD and ID, Age, Sex, TOD columns
patient_info <- patient_info_raw[!is.na(patient_info_raw$TOD), c(1,2,6,8)]
# Make TOD a positive number by adding 6
patient_info['TOD'] <- patient_info['TOD'] + 6
...
# Pivot samples dataframe to gene_ID and expression_level columns so I can extract string information in gene_ID
samples <- samples|> 
...
full_df <- samples |> 
  left_join(patient_info, by = c("patient_ID" = "ID")) |> 
  drop_na() |> 
...
final_data_BA11 <- full_df[full_df$BA_ID == 11, -1] # first column is Brain Area, which we do not need in final analysis
