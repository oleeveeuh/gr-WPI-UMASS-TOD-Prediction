...
def build_model(kernel_size, pool_size, filters):
    """
    ...
    model = tf.keras.Sequential([
        tf.keras.layers.Conv2D(filters=filters, kernel_size=kernel_size, activation='relu', input_shape=(7, 1, 1), padding='same'),
...
def apply_convolution_and_pooling(df, kernel_size, pool_size, filters, batch_size, seed=42):
    ...
    data = data.reshape(rows * cols, 7, 1, 1)

    model = build_model(kernel_size, pool_size, filters)

    # Test prediction to verify shape
    test_data = data[:batch_size]
    test_result = model.predict(test_data)
    print(f"Shape of the output after Flatten layer: {test_result.shape}")
    ...
    results = results.reshape(rows, cols * results.shape[-1])  # Flatten the results to store in DataFrame

    convolved_df = pd.DataFrame(results, index=df.index)

    return convolved_df

def process_and_save_data(input_path, output_path, window_size=3, kernel_size=(3, 1), pool_size=(3, 1), filters=3, batch_size=16, random_seed=42):
    """
...
datasets = ['BA11', 'BA47']
splits = ['test', 'train']
base_input_path = 'split_60/'
base_output_path = 'flatten_w3_conv/'
...
output_files = [f'{base_output_path}{dataset}_{ratio}_{dr_type}_{split}.csv'
                ...
                for dr_type in dr_types
                for split in splits]
# input_files = ['split_80/BA47_80_nonnormalized_test.csv']
# output_files = ['flatten_w2_conv/BA47_80_nonnormalized_test.csv']
